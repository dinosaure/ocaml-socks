...
type ('req_error, 'auth_error, 'addr_error) address_policy =
  ...
  (([> R.msg] as 'req_error, [> R.msg] as 'auth_error) auth_policy,
   ([> R.msg] as 'addr_error)) Lwt_result.t
...
let auth_allow_all (request_policy : 'req_err request_policy)
  ...
  fun _method_selection_request ->
    Lwt_result.return (
      ...
      | _ -> Lwt_result.fail
               (`Msg "invalid auth type passed for request for \
                      No_authentication_required")
    )

let auth_username_password auth_cb
    (request_policy : 'req_err request_policy)
  : ('req_err,[> R.msg]) auth_policy =
  fun method_selection_request ->
    if List.exists (function Username_password _ -> true
                           | _ -> false) method_selection_request then begin
...
let read_while_incomplete (type ok)
    channel (f:string ->
             (ok, [< `Incomplete_request | `Invalid_request]) Result.result)
...
let connect_with address port : (channel, [> R.msg]) Lwt_result.t =
  let ai_families , address = let open Lwt_unix in match address with
        ...
        fun req_ai_family ->
          Lwt.catch (fun () ->
              getaddrinfo address (string_of_int port)
                [AI_SOCKTYPE SOCK_STREAM ; AI_FAMILY req_ai_family] >|= function
              | {ai_protocol = 6 (*TCP*) ; ai_socktype = SOCK_STREAM ;
                 ai_family               ; ai_addr = ADDR_INET (addr,port) ;
                 ai_canonname = _ } :: _
                when ai_family = req_ai_family ->
                Some (addr,port)
              | _::_ ->
                Socks_log.debug (fun m ->
                    m "connect_with: Resolved sockaddr was not TCP \
                       / SOCK_STREAM / (IPv4|IPv6)");
                None
              | [] ->
                Socks_log.debug (fun m ->
                    m "connect_with: failed to get (%s socket) for resolve \
                       %s:%d" (match req_ai_family with PF_INET -> "IPv4"
                                                      | PF_INET6 -> "IPv6"
                                                      | PF_UNIX -> "UNIX" )
                      address port);
                None)
            (fun _ -> Lwt.return None))
    None ai_families >>= function
  | Some (addr,port) ->
    Socks_log.debug (fun m -> m "Connecting to %S:%d" address port) ;
    Lwt.catch
      (fun () -> Lwt_result.ok @@ Lwt_io.open_connection(ADDR_INET (addr,port)))
...
let forward_data (upstream_in,upstream_out) : client_data_cb =
  ...
  let handle_data fd_in fd_out =
    let rec loop () =
      ...
      | "" ->
        ...
        Lwt_io.flush upstream_out
      | msg ->
        Logs.debug (fun m ->m "-> %S" msg);
        Lwt_io.write fd_out msg >>= fun () ->
        loop ()
    in
    Lwt.catch (fun () ->
        Lwt_io.write upstream_out original_leftover >>= fun () ->
        loop ())
      (function
  ...
  in
    ...
    Lwt.pick [
      handle_data upstream_in client_out ;
      handle_data client_in upstream_out ;
    ] >>= fun _ ->
      Lwt_io.close upstream_in >>= fun () ->
      Lwt_io.close upstream_out
...
let reply_to_socks4_connect ((_, client_out):channel) resolved =
  Lwt_io.write client_out
    (match resolved with
     | Ok _ ->
       Socks_log.debug (fun m ->
           m "socks4: telling the client it went well: ");
...
let resolve_dns_and_forward_tcp : 'error request_policy =
  function
  | `Socks4 req ->
  ...
  | `Socks5 (Connect {address;port}) ->
    ...
    begin match upstream with
      | Ok upstream ->
        Lwt.return (Ok (forward_data upstream))
      | Error _ ->
        Lwt_result.fail (`Msg "can't connect to target address")
    end
    ...
    (fun sockaddr (client_channel:channel) ->
       ...
       (function
         ...
         | Ok auth_policy ->
           ...
           get_socks_connect_or_auth_request client_channel ""
           >>= fun req_res ->
           begin match req_res with
             | Error msg ->
               Socks_log.err (fun m -> m "aborting client connection while \
                                          reading request: %a" R.pp_msg msg) ;
               Lwt.return_unit
             | Ok (Socks4_request (socks4_request, req_leftover)) ->
               auth_policy [No_authentication_required;
                            Username_password ("","")]
               >>= begin function
                 | Ok ((No_authentication_required
                       | Username_password ("","") as selected_auth),
                       auth_cb) ->
                   auth_cb (match selected_auth with
                       | Username_password ("","") ->
                         Username_password (socks4_request.username,"")
                       | none -> none
                     ) >>= begin function
                     | Ok request_policy ->
                       request_policy (`Socks4 socks4_request)
                       >|= begin function
                 ...
                 | Ok _ ->
                   ...
                   Lwt_result.fail ()
                 | Error msg ->
                   Logs.err (fun m -> m "SOCKS4A auth select: %a" R.pp_msg msg);
                   Lwt_result.fail ()
               end
             ...
             | Ok (Socks5_method_selection_request (methods,method_leftover)) ->
               auth_policy methods >>= begin function
                 | Error (`Msg msg) ->
                   Lwt_result.fail (`Msg ("auth policy: " ^ msg))
                 | Ok (auth_method, verify_auth) ->
                   Lwt_io.write (snd client_channel)
                     (make_socks5_auth_response auth_method) >>= fun () ->
                   Lwt_io.flush (snd client_channel) >>= fun () ->
                   (match auth_method with
                    | No_authentication_required ->
                      let open Lwt_result in
                      verify_auth No_authentication_required >|= fun auth_res ->
                      auth_res, method_leftover
                    ...
                    | Username_password _ ->
                      read_while_incomplete client_channel
                        ...
                        | Error `Invalid_request ->
                          Lwt_result.fail (`Msg "error parsing user/pw request")
                      end
                   ) >>=
                   begin function
                     ...
                     | Ok (request_policy, auth_leftover) ->
                       Socks_log.debug (fun m ->
                           m "SOCKS5: got a client request policy, \
                              now reading request.") ;
                       read_while_incomplete client_channel
                       ...
                       >>= begin function
                         ...
                         | Ok (request, req_leftover) ->
                           ...
                           >>= begin function
                             | Ok client_cb ->
                               Socks_log.debug (fun m ->
                                   m "Client request validated, starting \
                                      data callback.");
                               Lwt_result.return
                                 (request,
               ...
               >>= begin function
                 | Ok (_request, data_cb) ->
                   data_cb >>= fun () ->
                   Lwt_io.flush (snd client_channel)
...
let easy_establish_server () =
  Lwt.wrap (fun () ->
      ...
      Lwt_unix.ADDR_INET
        (Unix.inet_addr_of_string "127.0.0.1", 1080)
    ) >>= fun server_addr ->
  establish_server server_addr
    (client_allow_localhost
       (auth_allow_all resolve_dns_and_forward_tcp))
